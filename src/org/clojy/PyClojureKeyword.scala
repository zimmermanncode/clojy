...
class PyClojureKeyword(override val clj: Keyword)
  extends PyClojureObject(clj) {

  override def __eq__(obj: PyObject) = new PyBoolean(obj match {

    case py: PyClojureObject =>
      this.clj.equals(py.clj)

    case _: PyString =>
      this.clj.getName == obj.toString
